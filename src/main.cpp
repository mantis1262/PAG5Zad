...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        shaderRef.setMat4("model", model);
        shaderRef.setMat4("view", view);
        shaderRef.setMat4("projection", projection);
        shaderRef.setVec3("cameraPos", camera.Position);
        // cubes
        glBindVertexArray(cubeVAO);
        ...
        weponShader.use();
        model = glm::mat4(1.0f);
        model = glm::translate(model, glm::vec3( camera.Position.x, camera.Position.y-0.3, camera.Position.z));

        model = glm::rotate(model,(AngleX),glm::vec3(0.0f,1.0f,0.0f));
        if(weapon == 2)
          model = glm::scale(model,glm::vec3(0.01f,0.01f,0.01f));

        weponShader.setMat4("model", model);
        weponShader.setMat4("view", view);
        weponShader.setMat4("projection", projection);
        weponShader.setVec3("cameraPos", camera.Position);
        if(weapon == 1)
         weapon1.Draw(weponShader);
        if(weapon == 2)
...
void processInput(GLFWwindow *window)
{
    ...
    {
        weapon=1;
        AngleX = -90.0f+(-0.01f*camera.Yaw);
    }
    if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS)
    {
        weapon=2;
        AngleX = -70.0f+(-0.01f*camera.Yaw);
    }
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
    ...
    if(weapon == 1 ) AngleX = -90.0f+(-0.01f*camera.Yaw);
    if(weapon == 2 ) AngleX = -70.0f+(-0.01f*camera.Yaw);
}
